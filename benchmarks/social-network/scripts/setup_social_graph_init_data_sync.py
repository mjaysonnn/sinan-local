...
def random_tweet_user_id():
  ...
  for k in sorted(list(tweet_num_prob_by_follower_num.keys())):
    chosen_key = k
    if coin <= cum_tweet_num_prob_by_follower_num[chosen_key]:
      break
...
def upload_follow(session, addr, user_0, user_1):
  ...
  r = session.post(f"{addr}/wrk2-api/user/follow", data=payload)
  print(r.status_code)
  # print(r.text)
...
def upload_register(session, addr, user):
  ...
  r = session.post(f"{addr}/wrk2-api/user/register", data=payload)
  print(r.status_code)
  # print(r.text)
...
def compose_post_for_each(session, addr):
  for i in range(1, 963):
    ...
    for _ in range(0, num_user_mention):
      mention_user_id = -1
      while True:
        mention_user_id = random.randint(0, 962)
        if mention_user_id != int(user):
          text += f" @username_{mention_user_id}"
          break
    ...
    if round(random.uniform(0.0, 1.0), 2) <= 0.3:
      ...
      for _ in range(0, num_urls):
        text += f" https://www.bilibili.com/av{random_digits(8)}"
    ...
    for _ in range(0, num_media):
      ...
      media_types += '\"png\",'
    media_ids = f'{media_ids[:-1]}]'
    media_types = f'{media_types[:-1]}]'
    ...
    r = session.post(f"{addr}/wrk2-api/post/compose", data=payload)
    print(r.status_code)
    print(r.text)
